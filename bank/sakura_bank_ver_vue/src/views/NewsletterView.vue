...
<template>
  <div class="space-y-6">
    <div>
      ...
      <p class="text-muted-foreground">당신의 소비 패턴을 분석한 맞춤형 절약 팁을 확인하세요</p>
    </div>
      ...
      <div class="flex items-start gap-4 p-6">
        ...
        <div>
          ...
          <p class="mt-1 text-sm text-muted-foreground">
            이번 주 소비 패턴을 분석한 결과, <span class="font-medium text-pink-600">식비</span>와
            <span class="font-medium text-amber-600"> 카페 지출</span>이 평소보다 높아요. 아래 맞춤
            팁을 확인해보세요!
          </p>
        </div>
      </div>
      ...
      <UiCard
        ...
        :class="
          cn(
            ...
            n.urgent && 'ring-2 ring-primary ring-offset-2'
          )
        "
      >
        <div
          class="pointer-events-none absolute inset-0 rounded-xl opacity-[0.02]"
          style="
        ...
        <div
          v-if="n.urgent"
          class="absolute -right-2 -top-2 rounded-full bg-primary px-3 py-1 text-xs font-medium text-primary-foreground"
        >
          추천
        </div>
        ...
        <div class="p-6 pb-2">
          <div class="flex items-center gap-3">
            <div :class="cn('rounded-xl p-2', n.color)">
        ...
        <div class="space-y-3 px-6 pb-6">
          ...
          <div class="flex flex-wrap gap-2">
            <span v-for="tag in n.tags" :key="tag" class="text-xs text-primary/80">{{ tag }}</span>
          </div>
          <UiButton variant="ghost" size="sm" class="mt-2 w-full justify-between">
            자세히 보기
    ...
    <UiCard class="border-none bg-card/80 shadow-sm backdrop-blur-sm">
      ...
      <div class="space-y-4 px-6 pb-6">
        <div class="flex items-center gap-4 rounded-xl bg-primary/5 p-4">
          <div
            class="flex h-12 w-12 items-center justify-center rounded-full bg-primary/20 text-2xl"
          >
            🍕
          </div>
          <div class="flex-1">
            <h4 class="font-medium text-foreground">배달 없는 일주일</h4>
            <p class="text-sm text-muted-foreground">이번 주 배달 앱 열지 않기. 예상 절약: 5만원</p>
          </div>
          <UiButton size="sm" class="rounded-full">도전하기</UiButton>
        </div>
        <div class="flex items-center gap-4 rounded-xl bg-amber-50 p-4">
          <div
            class="flex h-12 w-12 items-center justify-center rounded-full bg-amber-100 text-2xl"
          >
            ☕
          </div>
          <div class="flex-1">
            <h4 class="font-medium text-foreground">카페 3회 제한</h4>
            <p class="text-sm text-muted-foreground">
              이번 주 카페 방문 3회로 제한하기. 예상 절약: 2만원
            </p>
          </div>
          <UiButton size="sm" variant="outline" class="rounded-full">도전하기</UiButton>
        </div>
        <div class="flex items-center gap-4 rounded-xl bg-green-50 p-4">
          <div
          ...
          <div class="flex-1">
            <h4 class="font-medium text-foreground">도보 출퇴근 데이</h4>
            <p class="text-sm text-muted-foreground">
